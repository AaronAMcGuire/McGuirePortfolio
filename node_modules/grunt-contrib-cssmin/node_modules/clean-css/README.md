...
--skip-advanced                Disable advanced optimisations - ruleset reordering & merging
...
--skip-restructuring           Disable restructuring optimisations
...
* `advanced` - set to false to disable advanced optimisations - selector & property merging, reduction, etc.
...
* `restructuring` - set to false to disable restructuring in advanced optimisations
...
* `stats` - a hash of statistic information (if requested with `debug` option):
  ...
  * `timeSpent` - time spent on optimisations
...
* `'[+-]properties.colors'` - turn on / off any color optimisations
...
### What advanced optimisations are applied?

All advanced optimisations are dispatched [here](https://github.com/jakubpawlowicz/clean-css/blob/master/lib/selectors/advanced.js#L59), and this is what they do:
